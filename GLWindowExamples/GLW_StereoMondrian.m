function GLW_StereoMondrian(fullScreen, displayType)
% GLW_StereoMondrian(fullScreen, displayType)
%
% Description:
...
try
	...
	mondrianColors.left = c;
	mondrianColors.right = c;
	
	% Stereo displays require that the warp files are specified.
	warpFile.left = 'StereoWarp-left';
	warpFile.right = 'StereoWarp-right';
	...
	while true
		...
		switch key
			...
			case 'r'
				...
				win = setMondrianPatchColor(win, ...		% GLWindow object.
											'it', ...		% Mondrian name.
											randomRow, ...	% Row index.	
			...
			case 'l'
				...
				win = setObjectColor(win, 'it', c);
				
			% Sets the Mondrian colors to random values.
			case 'f'
				win = setObjectColor(win, 'it', rand(numRows, numCols, 3));
				
			% Randomize the border.
			case 'b'
				win = setObjectProperty(win, 'it', 'Border', rand);
			...
			case 'a'
				...
				for i = 1:(frameRate*duration)
					...
					win = setObjectProperty(win, 'it', 'Border', bval);
					draw(win);
					frame = frame + 1;
				end
		end
